...
- **SQL script-based fixes hit the right database** — `SqlScriptExecutor` resolves the target connection from the failure's `MonitoredJob.ConnectionName` (or `ConnectionName|SQL` payload prefix), not from `AiDbContext`. Supports `{failureId}` (int), `{sourceId}` (string — source row's natural key), and `{referenceId}` (string — related row's identity, e.g. parent/FK key, DB scans only) placeholders. Verified end-to-end against `B2B_Test.dbo.Files`.
...
**Last completed (2026-06-21):**

- **`scanRuleNeedsClassification()` Field=Value exact-match fix (job-config screen).** The ⚠ "No class rule" badge on ValueEquals scan-rule rows was using substring containment to test coverage, causing a false-positive: a classification rule with pattern `FileStatusCode=2` was considered to "cover" a scan rule for `FileStatusCode=22` because `"filestatuscode=22".includes("filestatuscode=2")` is true. Fix: when BOTH the scan-rule keyword (e.g., `FileStatusCode=22`) and the class-rule literal both contain `=`, the check uses strict equality (`keyword === literal`) instead of `includes`. Same fix applied to the `relevantDefaultClassRules` computed (see below). Bug was silent — operators saw no gap badge on `FileStatusCode=22` even though no matching classification rule existed.

- **`relevantDefaultClassRules` computed + `showAllDefaults` toggle (job-config screen).** The Classification Rules section's "{JobType} defaults" subsection previously showed ALL JobType-wide defaults regardless of whether they could fire for this job's scan rules. New computed `relevantDefaultClassRules` filters to only rules whose pattern overlaps at least one of the job's scan rules (using `classPatternForScanRule()` + Field=Value exact-match logic). Signal `showAllDefaults` + computed `displayedDefaultClassRules` back a "Show all N defaults / Show relevant only" toggle. Reduces visual noise on jobs with many JobType-wide defaults that aren't relevant to the job's actual scan checks.

- **Dev utility SQL scripts.** Three scripts added to `MaiaAIEngine.services/Scripts/` alongside `rebuild-datetime-defaults.sql`:
  - `clear-dev-data.sql` — deletes all operator-configured / test data in FK-safe order (FixExecutionLogs → OperatorActions → AuditLog → AIRecommendations → JobFailures → scan run history + watermarks → UserSessions → MonitoredJobRules → ClassificationRules → ScanCheckRules → ScanSources → MonitoredJobLeases → FixPolicyRules → MonitoredJobs → ErrorTypes). Preserves metadata: JobTypes, ScanTypes, Roles, Users. Optional `DBCC CHECKIDENT` lines commented out for ID reseeding.
  - `capture-job-config.sql` — captures a named job's full config as 6 result sets (job + JobType, sources + ScanType, scan rules, linked class rules + ErrorTypeCode, fix policy overrides + ErrorTypeCode, error types in use). Parameterized by `@jobName`. Run BEFORE `clear-dev-data.sql` to snapshot a job's config before clearing.
  - `seed-nsg-events.sql` — production-safe idempotent seed for the `Nsg-Events` job. No hardcoded IDs — resolves JobType/ScanType by name with `RAISERROR` guards; ErrorTypes by Code; all subsequent inserts use `IF NOT EXISTS` + `SCOPE_IDENTITY()` chaining. Seeds 2 ErrorTypes (FailedStatus Critical, FailedFeedback Medium), 1 MonitoredJob + Lease, 1 ScanSource, 3 ScanCheckRules, 3 ClassificationRules, 3 MonitoredJobRules links, 1 FixPolicyRule (auto-heal EventStatusCode → 5 for FailedStatus; FailedFeedback is classification-only, no automated fix). Trailing verification SELECT.
...
- **Config screen bug fixes and UI polish.**
  ...
  - **`Severity.Critical` added to enum** — `Core/Enums/Severity.cs` was missing `Critical`; the controller already referenced it in the 400 message and the frontend `SEVERITIES` list already offered it. No migration needed (stored as `nvarchar`).
...
- **Job-config screen — Round 1 UX polish + bug fixes (frontend only, `JobConfigComponent`).**
  ...
  - **Scan-rule ⚠ heuristic** — `scanRuleNeedsClassification()` is type-aware. `ErrorKeyword` and `FileContent` rules: fires only when `effectiveClassRules().length === 0` (the operator has set up no classification at all). Substring overlap is NOT checked — a broad keyword like `*Error*` captures lines of any shape, so partial class-rule coverage is expected and the `/unconfigured` screen handles the gap analysis. `ValueEquals` / `ColumnRange` rules: the overlap check compares the scan-rule keyword (e.g., `Field=Value`) against each class-rule's stripped literal — when BOTH sides contain `=`, exact equality is required (`keyword === literal`); otherwise substring containment (`keyword.includes(literal)`). The equality branch prevents `FileStatusCode=2` from falsely covering `FileStatusCode=22` (see 2026-06-21 fix). `SqlQuery` always returns false (output shape is unpredictable).
...
- **`DbFixHandler.HandleAsync` is a TODO stub returning `false`.** Surfaced during the JobTypeId fix — when a `DbFix`-category recommendation has no matching `FixPolicyRule`, execution falls through to this handler and silently fails. Either implement it or flip those failures to `ManualRequired` explicitly with a clearer message.
  - **The scope-token write-guard is now SHIPPED for SqlScript fix payloads (2026-06-11, extended 2026-06-20)** — see the SqlScript write-guard and `{referenceId}` decision sections below. Guard accepts `{sourceId}` OR `{referenceId}` in WHERE/EXEC. When `DbFixHandler` proper is implemented it should reuse the same `ISqlFixScopeValidator`. `SourceId` is the natural key into the operator's table (invoice/order id) — what a fix's WHERE must target; `FailureId` is MAIA's internal PK and meaningless as a fix target (and deliberately does NOT satisfy the guard).
...
## Job-config gap-badge heuristics — Field=Value exact equality (2026-06-21)

- **`Field=Value` patterns require exact equality in gap-badge overlap checks, not substring containment.** Both `scanRuleNeedsClassification()` (the ⚠ "No class rule" badge on scan-rule rows) and `relevantDefaultClassRules` (the defaults subsection filter) use the same overlap logic: if BOTH the scan-rule keyword and the class-rule pattern literal contain `=`, the comparison is `keyword === literal`; if only one side contains `=` (or neither does), the original `keyword.includes(literal)` substring check is used. **Why equality when both contain `=`:** the `=` character is the separator in a Field=Value pattern (e.g., `FileStatusCode=22`). Using substring containment across `=` would make the literal `FileStatusCode=2` cover the keyword `FileStatusCode=22` (because `"filestatuscode=22".includes("filestatuscode=2")` is true) — a numeric-prefix false positive that silently suppresses a legitimate gap badge. Strict equality keeps patterns orthogonal: `FileStatusCode=2` covers only its exact match, not any value that starts with `2`. The fix is purely in the Angular computed logic; no backend change.
...
- **Algorithm:** strip the optional `ConnectionName|SQL` prefix → substitute `{sourceId}` OR `{referenceId}` (both case-insensitive, both map to the same sentinel `__MAIA_SRC__`) → parse with `TSql150Parser` → for **every** statement: UPDATE/DELETE must have the sentinel **inside its `WhereClause`** (token-span scan), EXEC must have it **inside the EXEC fragment** (a parameter); any other statement type (SELECT, etc.), an unparseable script, or a missing/sentinel-free WHERE → reject. Multi-statement batches must have *every* statement scoped. `{failureId}` deliberately does NOT count (MAIA's PK, not a key into the operator's table). **Critical security property:** "has a WHERE" ≠ "scoped by a captured identity" — a WHERE that references neither `{sourceId}` nor `{referenceId}` is still rejected. Pinned by the `WhereWithoutEitherScopeToken_Rejected` test.
...
## `{referenceId}` placeholder for DB scans (SHIPPED 2026-06-20)

Extends the placeholder/write-guard system with a second identity token for the "fix many child rows via a parent key" pattern (e.g. cancel all line items for an order when an order-level DB scan detects the order is bad).

- **`ScanCheckRule.ReferenceIdColumn` (nullable, DB scans only)** — column name in the result set (SqlQuery `SELECT`, or the ColumnRange/ValueEquals-generated SELECT) whose value is captured into `JobFailure.ReferenceId`. Both columns are `nvarchar(500)/nvarchar(200)`. Migration `AddReferenceIdColumn` adds two nullable columns; no backfill needed.
- **Silent-null on missing column (v1, intentional).** `DatabaseScanStrategy` uses `row.TryGetValue(rule.ReferenceIdColumn!, out var rv)` — if the configured column is absent from the result set, `ReferenceId` is null and `{referenceId}` resolves to empty string. A fix scoped by empty `{referenceId}` produces `WHERE col = ''` → zero rows updated → safe no-op, not a bulk write. Operators who see "fix updates nothing" should verify the column appears in their SELECT. Contrast with `TargetField`/`WatermarkColumn`, which throw on missing-column (they are mandatory/authoritative). The distinction is deliberate: `ReferenceIdColumn` is supplementary identity, opt-in; the others are the detection/dedup backbone.
- **`{referenceId}` placeholder** — `IPlaceholderResolver` (case-insensitive switch): `"referenceid" => failure?.ReferenceId ?? string.Empty`. Resolves to empty for non-DB failures or DB failures where `ReferenceIdColumn` is unset. All executors pick it up for free (routed through `PlaceholderResolver`).
- **Write-guard extended: `{sourceId}` OR `{referenceId}` both satisfy the scope invariant.** `SqlFixScopeValidator.ScopeToken` regex now matches either token (`\{sourceId\}|\{referenceId\}`, case-insensitive, compiled). Both substitute to the same sentinel `__MAIA_SRC__` before the ScriptDom AST walk — the sentinel-in-WHERE check is unchanged. **The invariant is still "scoped by a captured identity", not "has a WHERE":** a WHERE clause that references neither token is rejected even though a WHERE clause is present. This is the `WhereWithoutEitherScopeToken_Rejected` test (explicit pin of the sneaky case, not subsumed by the old `WhereWithoutSourceId_Rejected`).
- **One-second-identity principle:** `{sourceId}` = the direct row's key (fix that specific row); `{referenceId}` = a parent/FK key (fix all child rows belonging to the parent). Using `{referenceId}` in a WHERE is still a *bounded* write — bounded by whatever rows share the parent key, not unbounded across the whole table. Operators choose which identity to scope by based on the fix's target table; both are valid per the guard.
- **v1 scope:** DB scans only. `ReferenceId` is null on all FS/FileContent failures. No UI field for non-DB scan rules (the DB rule drawer has it; FS/FileContent drawers do not). Token legend in the Fix drawer lists `{referenceId}` with the note "needs Reference ID Column on the scan rule."
