...
- ~~Authn / authz on controllers + real operator identity replacing hardcoded
  `'operator'`~~ **SHIPPED 2026-06-19** (Auth v1 Phases 0–3 — see entry below).
  Remaining: **Phase 4** (cosmetic role-gating of menus/buttons) + a Users admin
  screen (no UI for user CRUD yet — admin uses `POST /api/users` directly).
...
- **Authentication & Authorization v1 — Phases 0–3 SHIPPED (Phase 4 cosmetic-gating pending).** MAIA-local username/password login, server-side sessions in an httpOnly cookie, role-tiered API enforcement. 240 backend tests green (incl. an exhaustive authorization matrix); Angular builds clean; cookie flow + actor stamping live-verified.
  - **Schema (Phase 0):** new `Roles` (seed: 1=User, 2=Operator, 3=Administrator; `RoleId == (int)MaiaRole`), `Users` (unique-CI Username, PBKDF2 `PasswordHash`, RoleId FK, IsActive, MustChangePassword, CreatedAt, LastLoginAt), `UserSessions` (unique Token, UserId FK cascade, LastActivityAt). Migration `AddAuthTables` seeds the 3 roles via `HasData` and the bootstrap admin via raw `INSERT` (random PBKDF2 salt → non-deterministic, can't use `HasData`). **Default admin: `admin` / `admin`, RoleId=3, MustChangePassword=1.** `Microsoft.Extensions.Identity.Core` added to Infrastructure for `PasswordHasher<T>`.
  ...
  - **MustChangePassword is a SOFT, skippable prompt (per operator request 2026-06-19).** The original hard-blocking `MustChangePasswordMiddleware` was **removed**; the flag is now only a one-time UI nudge at first login (Change *or* **Skip for now** → `POST /api/auth/dismiss-password-change` clears it). The guard no longer force-redirects on it. **Trade-off accepted for dev/demo: `admin`/`admin` can persist if skipped — tighten before real deployment.**
...
- **Job-config screen — Round 1 UX polish + bug fixes (frontend only, `JobConfigComponent`).**
  ...
  - **Scan-rule ⚠ heuristic** — `scanRuleNeedsClassification()` uses keyword-overlap: strips `*` from `targetField` and each class rule's pattern, checks substring containment in either direction. Fires when no effective class rule's literal overlaps with the rule's keyword — not just when the effective list is empty.
...
- **🏗️ ARCHITECTURE — heterogeneous scan rules under one MonitoredJob — DECIDED: Tier 2.5, SHIPPED.**
  ...
  **Phasing (all phases complete):** **(a)** schema + backfill — `ScanSources` table + `ScanSourceId` on 5 tables. **(b)** worker + `IScanStrategy` contract. **(c)** read-side rollup. **(d)** config UI/UX rework (dedicated per-job screen). **(e/cleanup — DONE 2026-06-17)** `ScanSourceId` enforced NOT NULL on all 5 tables (migration `Tier25CleanupColumns`), 7 legacy `MonitoredJob` scan-config columns dropped (`LogFolder`, `SearchPatterns`, `ConnectionName`, `LogSourceUrl`, `ScanTypeId` vestigial FK left in place as a deploy-time holdover — see decision below). **Per-source leases remain cleanly deferrable** — re-key the lease FK + claim JOIN as an isolated later change if cadence divergence ever becomes real.
...
- ~~**No authn/authz on any controller.**~~ **RESOLVED 2026-06-19** — Auth v1 Phases 0–3 (MAIA-local login, httpOnly server-side sessions, three role tiers enforced per-endpoint with a default-closed fallback). Remaining auth work: **Phase 4** cosmetic role-gating of menus/buttons, a **Users admin UI** (CRUD is API-only today), AD authn front-end (v2), audit-viewing UI, self-service email password reset (v2), and a `UserSessions` sweep worker if the table grows.
...
- **A `MonitoredJob` is PURE IDENTITY; ScanType lives only on its `ScanSources` (Tier 2.5 Option 1, chosen over Option 2's seed-and-freeze, 2026-06-15).** Rejected Option 2 (keep job-level scan fields, auto-seed a source on create) because `job.ScanTypeId` would go **stale** the instant sources change (delete FS source + add DB source → job still says FS) — the same stored-value-drifts-from-reality bug class as the classifier linked-only semantics and the JobTypeId snapshot. The fix is to not have the field be authoritative: a job carries Name/DisplayName/JobType/polling/IsActive/Description; sources carry ScanType + folder/pattern/connection/url + rules. **App changes (column NOT dropped — that's the deferred cleanup migration):** `CreateJob` sets `ScanTypeId=1` as a vestigial FK-satisfying placeholder and ignores the request's scan fields; `UpdateJob` no longer touches the scan columns (existing backfilled values preserved); the worker + `JobScanController` already dispatch by `source.ScanType` (the latter's cosmetic `agg.ScanType` now derives from the first active source). **Frontend:** the add/edit job drawer is identity-only; **create is two-step** — bare job → auto-navigate to its `/config/monitored-jobs/:id` Configure screen → "Add Source" empty state (d2). The Monitored Jobs list derives its icon + type label + config chips from `job.sources` via the shared `core/util/scan-type-label.util` (`scanTypeLabelFromNames` / `scanIconForSources`) — the SAME helper the dashboard compact rows use, so a single/multi/mixed-source job reads identically in both places.
- **Two latent lease bugs fixed as a side effect of Option 1 (surfaced by the lease-query investigation, 2026-06-15).** (1) A mixed-source job leased at its *seeded* job-level ScanType's duration, not its longest source's — e.g. job 1005 (FileContent+FS+DB) leased at FileContent's 300s while its DB source needs 1800s, risking mid-scan lease theft on the long source. MAX-over-active-sources gives the correct 1800s. (2) Zero-source jobs (e.g. 1006) were claimed every tick and immediately released after "no active scan sources — nothing to scan" — futile churn; the `IS NOT NULL` guard now excludes them from the claim entirely. Both were pre-existing, masked by the dual-level ScanType design.
