...
- **`DbFixHandler.HandleAsync` is a TODO stub returning `false`.** Surfaced during the JobTypeId fix — when a `DbFix`-category recommendation has no matching `FixPolicyRule`, execution falls through to this handler and silently fails. Either implement it or flip those failures to `ManualRequired` explicitly with a clearer message.
  - **When built, the DbFix payload MUST be validated to reference `{sourceId}` (NOT `{failureId}`) in its scoping clause.** `SourceId` is the natural key into the operator's table (invoice/order id) — what a fix's WHERE must target; `FailureId` is MAIA's internal PK and meaningless as a fix target. New save-time check `DbFixRequiresSourceIdInWhere`: substitute `{sourceId}` → a sentinel (`__MAIA_SRC__`), parse with **`Microsoft.SqlServer.TransactSql.ScriptDom`** (the official T-SQL parser, not regex), and require the sentinel to appear **inside the `WhereClause` AST node** (UPDATE/DELETE) or an **`ExecuteParameter`** (EXEC proc). Error: *"DbFix must reference {sourceId} in the WHERE clause to target the specific failure's row. Bulk fixes without per-failure scoping are not allowed."* **Honest limits (document in operator help):** the AST check confirms the placeholder is *structurally present*; it does NOT catch deliberate bypasses (tautologies like `OR '{sourceId}'='{sourceId}'`, or a present-but-non-constraining placeholder) — that needs constant-folding/evaluation, out of scope. It catches accidental missing-WHERE / obviously-bulk fixes; deliberate sabotage is covered by the trust model + audit logs, not the parser. **Apply the same guard to the existing live `SqlScriptExecutor` fix path** — it already string-substitutes `{sourceId}`/`{failureId}` into raw SQL run against the source DB with **no WHERE guard today**, so the write-risk is live now, not only in the future DbFixHandler. (Distinct from SqlQuery *scans*, which are reads — guarded by a least-privilege read-only login + row cap + timeout, not by this WHERE check.)
...
- **Short, stable label for StepName + dedup key.** `SourceTable` (the query) is too big for `JobFailure.StepName` (`nvarchar(200)`), so SqlQuery failures use `Description ?? "SqlQuery #{ruleId}"` for **both** `StepName` and the no-watermark `HasOpenFailureAsync` dedup key; `SourceLogPath = "db://{conn}/query"`. ColumnRange/ValueEquals behavior is byte-for-byte unchanged (their key stays the table name). `ErrorMessage = "{Description|'SqlQuery match'}: [{TargetField}] = {value} ({rowId})"` — predictable so classification patterns can match it.
...
- **Watermarking deferred (v1: `WatermarkColumn` NULL → full re-execute every tick).** Gated cleanly by the existing `rule.WatermarkColumn is not null` checks — no watermark-infra impact. v2: operator-declared incremental marker column.
