...
- **Per-rule scan resilience — DatabaseScanStrategy + FileSystemScanStrategy DONE; FileContentScanStrategy still partial (the one remaining follow-up).** The latent bug (one throw mid-loop aborts the source scan and orphans the failures earlier items already created, because `classify` runs only *after* the whole loop) was fixed in two strategies by the same pattern: wrap each item, remember the first error, classify/suggest whatever was created, then rethrow so the scan-run is still recorded Failed (visible). Status: **DatabaseScanStrategy — fixed** (per-rule, 2026-06-14). **FileSystemScanStrategy — fixed 2026-06-15**: per-file try/catch in keyword mode (the `IOException`-on-a-locked/rotating-log trigger) AND per-pattern try/catch in full-pipeline mode; 3 new unit tests (`FileSystemScanStrategyTests` — happy-path keyword match, no-match, and the resilience test where one file's faulted `SaveAsync` doesn't stop the other file from being created+classified, scan rethrows). **FileContentScanStrategy — STILL PARTIAL**: its inner loop catches only `FileContentTooLargeException`; any other throw (extractor error, `SaveAsync`) still aborts + orphans earlier files. Fix it the same way when next in that file (wrap the per-file body, broaden beyond the oversize catch, classify partial, rethrow). **ApiEndpointScanStrategy — not affected** (single check → one failure → classify inline; already wraps the HTTP call).
