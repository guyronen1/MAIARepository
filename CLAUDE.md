...
- **Per-rule scan resilience is only in DatabaseScanStrategy — FileSystem + FileContent share the latent "one throw wedges the source + orphans earlier failures" bug (confirmed 2026-06-15, NOT yet fixed).** `DatabaseScanStrategy` was made per-rule resilient (try/catch per rule, classify/suggest on partial results, rethrow after — see the per-rule-resilience decision). A code review of the other three found the same loop shape (accumulate `created` across items, `classify` only *after* the whole loop), so an unhandled throw mid-loop aborts the scan and leaves every already-created failure unclassified, and the source fails every tick. Status per strategy: **FileSystemScanStrategy — no try/catch anywhere** in `pattern→file→rule→line`; the likely trigger is `IOException` from `ReadNewContentAsync` on a **locked/rotating log file** (a common real condition) → highest risk. **FileContentScanStrategy — partial**: the inner loop catches only `FileContentTooLargeException`; any other throw (extractor error, `SaveAsync`) still aborts + orphans earlier files. **ApiEndpointScanStrategy — not affected**: single check → one failure → classify inline, no accumulate-then-classify loop, and it already wraps the HTTP call. Fix when next in those files: mirror the DB pattern — wrap each file/rule iteration, remember the first error, classify/suggest whatever was created, then rethrow so the scan-run is still recorded Failed (visible). FS is worth doing first.
